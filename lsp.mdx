...
| Swift | [sourcekit-lsp](https://github.com/swiftlang/sourcekit-lsp) | Ships with Xcode / the Swift toolchain |
| C / C++ | [clangd](https://clangd.llvm.org/) | `brew install llvm` |
| Ruby | [ruby-lsp](https://github.com/Shopify/ruby-lsp) (preferred) | `gem install ruby-lsp` |
| Ruby | [solargraph](https://solargraph.org/) (fallback, used only if ruby-lsp isn't found) | `gem install solargraph` |
...
- Every installed Ruby version's `bin/` dir under `~/.rbenv/versions/*` (newest first), plus `~/.gem/ruby/bin` and `/opt/homebrew/opt/ruby/bin`, covers gem-installed servers (ruby-lsp, solargraph) under rbenv or a plain Homebrew Ruby
...
| Swift | [lldb-dap](https://lldb.llvm.org/) | Ships with Xcode Command Line Tools, or `brew install llvm` | Swift binaries are natively LLDB-debuggable, so this reuses the exact same adapter as Rust/Zig. Requires a binary already built with `swift build`, Coder does not build it for you. |
| C / C++ | [lldb-dap](https://lldb.llvm.org/) | Ships with Xcode Command Line Tools, or `brew install llvm` | Same adapter as Rust/Zig/Swift. Requires a binary already built with debug symbols (`gcc`/`clang -g`), Coder does not build it for you, and there's no standard output path to guess, so point "Program" at your binary directly. |
| Ruby | [rdbg](https://github.com/ruby/debug) (`rdbg --stdio`) | Ships with Ruby 3.1+'s bundled `debug` gem, or `gem install debug` | Pure stdio DAP, no separate build step, rdbg runs your script directly. |
